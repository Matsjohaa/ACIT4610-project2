...
def nondominated(points: np.ndarray) -> np.ndarray:
    """Mask of nondominated points (minimization)."""
    if len(points) == 0:
    ...
    for attr in ("MOEA_ALGORITHM", "ALGORITHM", "PREFERRED_ALGORITHM"):
        if hasattr(moea, attr):
            try:
                ...
                continue
    # If nothing is available, proceed; algorithm must be passed via args.
...
def run_once(internal_name: str, preset_name: str, algorithm: str, seed: int) -> Tuple[np.ndarray, float, int]:
    ...
    # Per-run nondominated filter (minimization)
    ...
    for internal_name, instance_display, category in selected:
        for preset_name in PRESETS:
            for algo in ALGORITHMS:
                ...
                percell_fronts[key] = []
                seed_shift = (hash(preset_name) % 10000)
                for seed in SEEDS:
                    # -- run once (this enforces algorithm from ALGORITHMS) --
                    adj_seed = int(seed + seed_shift)
                    P, wall, evals = run_once(internal_name, preset_name, algo, adj_seed)
                    ...
                    if len(P):
                        ...
                        # write an empty frame to keep the run visible
